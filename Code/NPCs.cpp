...
bool ShopkeeperNPC::attemptPurchase(Player& player, int index) {
	...
	const Item* item = inventory[index].get();
	if (!item) return false;
	
	int price = item->getPrice(); // Add price to ItemList.txt and modify parser to add a price tag to every item
	
	if (player.gold < price) return false;
	
	player.gold -= price;
	player.inventory.addItem(item->clone()); // Clone should be in Item
	return true;
}
